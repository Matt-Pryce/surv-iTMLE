...
#' @description Estimates the conditional difference in survival probabilities between exposed and
#'   unexposed groups using a survival targeted maximum likelihood estimator (surv-iTMLE). Nuisance
#'   models are fit via cross-fitting, predictions are targeted via a sieve-based fluctuation step,
#'   and pseudo-outcome regression yields the final CATE estimates.
#'
#' @param data A data frame containing all variables required for the analysis.
#' @param estimand The causal estimand to target. Currently only \code{"Difference"} is supported.
...
#' @param truncation Name of the column containing left-truncation times, or \code{NULL} (default)
#'   if there is no left truncation.
#' @param time_cuts A numeric vector of time points at which nuisance predictions are evaluated and
#'   the targeting step is applied. If \code{NULL} all unique observed event times are used.
#'   Values over 100 will trigger a runtime warning.
#' @param splits Number of cross-fitting splits. Valid values: \code{1} (no cross-fitting) or
#'   \code{10}.
#' @param e_covariates Character vector of covariate names for the propensity score model.
#' @param e_method Statistical method for the propensity score model. One of
#'   \code{"Parametric"} (logistic regression), \code{"Super learner"}.
#' @param e_SL_lib SuperLearner library to use when \code{e_method = "Super learner"}.
#' @param out_covariates Character vector of covariate names for the outcome (survival) model.
#' @param out_method Statistical method for the outcome model. One of \code{"Parametric"}
#'   (Cox PH), \code{"Super learner"}, \code{"Local survival stack"}, \code{"Global survival stack"}.
#' @param out_SL_lib SuperLearner library to use when \code{out_method = "Super learner"}.
#' @param g_covariates Character vector of covariate names for the censoring model.
#' @param g_method Statistical method for the censoring model. One of \code{"Parametric"},
#'   \code{"Super learner"}, \code{"Local survival stack"}, \code{"Global survival stack"}.
#' @param g_SL_lib SuperLearner library to use when \code{g_method = "Super learner"}.
#' @param h_covariates Character vector of covariate names for the left-truncation weight model,
#'   or \code{NULL} (default) if no truncation weight model is required.
#' @param h_method Statistical method for the left-truncation weight model. One of
#'   \code{"Parametric"}, \code{"Local survival stack"}. Only used when \code{h_covariates}
#'   is not \code{NULL}.
#' @param h_SL_lib SuperLearner library for the truncation weight model (unused unless
#'   \code{h_method = "Super learner"}).
#' @param iso_reg Logical. If \code{TRUE}, applies isotonic regression to smooth the estimated
#'   survival curves. Default \code{FALSE}.
#' @param pse_covariates Character vector of covariate names for the pseudo-outcome regression model.
#' @param pse_approach Approach used for the pseudo-outcome regression step. One of
#'   \code{"Pooled - Factor"} (time included as a factor), \code{"Pooled - Continuous"} (time
#'   included as a continuous variable), \code{"None"} (no pseudo-outcome regression).
#' @param pse_method Statistical method for the pseudo-outcome regression. One of
#'   \code{"Parametric"}, \code{"Random forest"}, \code{"GAM"}, \code{"Super learner"}.
#' @param pse_SL_lib SuperLearner library to use when \code{pse_method = "Super learner"}.
#' @param newdata A data frame on which final predictions are made. Typically the same as
#'   \code{data}.
#' @param target_option Targeting step specification. One of \code{"Lasso - Linear - Option 1"},
#'   \code{"Lasso - Linear - Option 2"}, \code{"Lasso - Linear - Option 3"}.
#' @param sieve_dim Integer dimension of the sieve basis used in the targeting step. If \code{NULL}
#'   (default) the dimension is set automatically as \code{ceiling(n^(1/3) * p)}.
#' @param sieve_interaction Integer interaction order for the sieve basis. If \code{NULL}
#'   (default) interaction order 2 is used.
#' @param CI Logical. If \code{TRUE}, bootstrap confidence intervals are computed. Default
#'   \code{FALSE}.
#' @param num_boot Number of bootstrap samples used when \code{CI = TRUE}. Default \code{200}.
#' @param mem_limit_gb Memory limit in gigabytes used to guard against excessively large nuisance
#'   prediction datasets. The function will stop with an informative error if the estimated peak
#'   memory exceeds this value. Default \code{4}.
#'
#' @return A list containing: CATE estimates, the dataset used to train the learner, and the
#'   dataset containing all pseudo-outcome predictions.
...
surviTMLE_learner <- function(data,
  ...
  if (!estimand %in% c("Difference"))
    stop(paste0('estimand must be "Difference". Got: "', estimand, '"'))

  if (!splits %in% c(1, 10))
    stop(paste0('splits must be 1 or 10. Got: ', splits))

  if (!e_method %in% c("Parametric", "Super learner"))
    stop(paste0('e_method must be one of: "Parametric", "Super learner". Got: "', e_method, '"'))

  if (!out_method %in% c("Parametric", "Super learner", "Local survival stack", "Global survival stack"))
    stop(paste0('out_method must be one of: "Parametric", "Super learner", "Local survival stack", ',
                '"Global survival stack". Got: "', out_method, '"'))

  if (!g_method %in% c("Parametric", "Super learner", "Local survival stack", "Global survival stack"))
    stop(paste0('g_method must be one of: "Parametric", "Super learner", "Local survival stack", ',
                '"Global survival stack". Got: "', g_method, '"'))

  if (!is.null(h_covariates) && !h_method %in% c("Parametric", "Local survival stack"))
    stop(paste0('h_method must be one of: "Parametric", "Local survival stack". Got: "', h_method, '"'))

  if (!pse_approach %in% c("Pooled - Factor", "Pooled - Continuous", "None"))
    stop(paste0('pse_approach must be one of: "Pooled - Factor", "Pooled - Continuous", "None". ',
                'Got: "', pse_approach, '"'))

  if (!pse_method %in% c("Parametric", "Random forest", "GAM", "Super learner"))
    stop(paste0('pse_method must be one of: "Parametric", "Random forest", "GAM", "Super learner". ',
                'Got: "', pse_method, '"'))

  if (!target_option %in% c("Lasso - Linear - Option 1", "Lasso - Linear - Option 2", "Lasso - Linear - Option 3"))
    stop(paste0('target_option must be one of: "Lasso - Linear - Option 1", "Lasso - Linear - Option 2", ',
                '"Lasso - Linear - Option 3". Got: "', target_option, '"'))
  ...
  # Update covariate vectors with sanitized/expanded versions from data management
  e_covariates   <- clean_data$e_covariates
  out_covariates <- clean_data$out_covariates
  g_covariates   <- clean_data$g_covariates
  h_covariates   <- clean_data$h_covariates
  pse_covariates <- clean_data$pse_covariates

  analysis_data <- clean_data$data
  analysis_data_long_all <- clean_data$data_long_all

  #------------------------------#
  #--- Time cuts memory check ---#
  #------------------------------#
  {
    n_obs        <- nrow(analysis_data)
    k            <- length(clean_data$evt_times_uni)
    approx_cols  <- if (clean_data$LT_data == 1L) 22L else 19L
    est_peak_gb  <- (2 * n_obs * k * splits * approx_cols * 8) / 1e9
    k_max        <- floor(mem_limit_gb * 1e9 / (2 * n_obs * splits * approx_cols * 8))

    if (est_peak_gb > mem_limit_gb) {
      stop(paste0(
        "time_cuts is too large for the current settings.\n\n",
        "  Requested time cuts  : k = ", k, "\n",
        "  Estimated peak memory: ~", round(est_peak_gb, 1), " GB\n",
        "  Memory limit         : ", mem_limit_gb, " GB  (set via mem_limit_gb)\n\n",
        "Memory is driven by the nuisance-model prediction dataset, which has\n",
        "  n x k x splits x ~", approx_cols, " columns x 8 bytes per cell,\n",
        "  x 2 for the temporary copy made during cross-fit accumulation.\n\n",
        "  n (subjects)  = ", n_obs, "\n",
        "  k (time cuts) = ", k, "\n",
        "  splits        = ", splits, "\n\n",
        "For these settings the maximum number of time cuts is: k_max = ", k_max, ".\n",
        "A smaller n or fewer splits allows more time cuts; more of either requires fewer.\n\n",
        "Options:\n",
        "  - Reduce time_cuts (e.g. increase the step size in seq())\n",
        "  - Reduce splits\n",
        "  - Increase mem_limit_gb if more RAM is available on your machine"
      ))
    }
  }

  if (length(clean_data$evt_times_uni) > 100) {
    warning(paste0(
      "time_cuts has ", length(clean_data$evt_times_uni), " values (> 100). ",
      "This may lead to the function taking a long time to run."
    ), immediate. = TRUE)
  }
  ...
  for (i in 0:(splits-1)){
    ...
    tryCatch(
      ...
      error=function(e) {
        stop(paste("An error occurred when collecting data for training nuisance models in split ",i,sep=""))
        print(e)
      }
    )
    ...
    tryCatch(
      ...
      #if an error occurs, tell me the error
      error=function(e) {
        stop(paste("An error occurred when collecting data for nuisance models predictions in split ",i,sep=""))
        print(e)
      }
    )
    ...
    if (clean_data$LT_data == 1){
      tryCatch(
        ...
        #if an error occurs, tell me the error
        error=function(e) {
          stop(paste("An error occurred in truncation weight model function in split ",i,sep=""))
          print(e)
        }
      )
    }
    ...
    tryCatch(
      ...
      #if an error occurs, tell me the error
      error=function(e) {
        stop(paste("An error occurred in propensity score model function in split ",i,sep=""))
        print(e)
      }
    )
    ...
    tryCatch(
      ...
      #if an error occurs, tell me the error
      error=function(e) {
        stop(paste("An error occurred in outcome model function in split ",i,sep=""))
        print(e)
      }
    )
    ...
    tryCatch(
      ...
      #if an error occurs, tell me the error
      error=function(e) {
        stop(paste("An error occurred in censoring model function in split ",i,sep=""))
        print(e)
      }
    )
    ...
    if (clean_data$LT_data == 1){
      tryCatch(
        ...
        #if an error occurs, tell me the error
        error=function(e) {
          stop(paste("An error occurred in truncation model function in split ",i,sep=""))
          print(e)
        }
      )
      ...
      num_tp <- length(clean_data$evt_times_uni)
      n_ids  <- length(unique(po_data_long_all$ID))

      #--- Vectorised: product[i,t] = sum_s G_k_Q[i,t,s] * trunc_k_haz[i,s] ---#
      G_mat      <- as.matrix(po_data_long_all[, paste0("G_k_Q", 1:num_tp)])
      H_mat      <- matrix(po_data_long_all$trunc_k_haz, nrow = n_ids, ncol = num_tp, byrow = TRUE)
      H_expanded <- H_mat[rep(seq_len(n_ids), each = num_tp), , drop = FALSE]
      po_data_long_all$product <- rowSums(G_mat * H_expanded)

      #--- Drop wide G_k_Q columns now that product has been computed ---#
      po_data_long_all <- po_data_long_all[, !grepl("^G_k_Q", names(po_data_long_all))]
    }
    ...
    if (i==0){
      ...
      for (j in 1:length(clean_data$evt_times_uni)){
        #Loading estimates — keep only columns required downstream
        est_all <- outcome_models$Surv_t_pred_long_all_list[[j]]
        Surv_t_pred_long_all_list_complete[[j]] <- est_all[, c("ID", "time",
          "S_t_pred_long_all_0", "S_t_pred_long_all_1", "S_t_pred_long_all_a")]
      }
    }
    else {
      ...
      for (j in 1:length(clean_data$evt_times_uni)){
        #Merging to previous estimates — keep only columns required downstream
        est_all <- outcome_models$Surv_t_pred_long_all_list[[j]]
        Surv_t_pred_long_all_list_complete[[j]] <- rbind(
          Surv_t_pred_long_all_list_complete[[j]],
          est_all[, c("ID", "time",
            "S_t_pred_long_all_0", "S_t_pred_long_all_1", "S_t_pred_long_all_a")]
        )
      }
  ...
  #--- Free nuisance model objects no longer needed ---#
  rm(outcome_models, censoring_model, po_data_long_all,
     po_o_data_long_all, po_g_data_long_all, po_e_data_long_all)
  if (exists("po_h_data_long_all")) rm(po_h_data_long_all)
  gc()
  ...
  #--- Pre-compute sieve basis once on full dataset (row-wise transform, safe to subset later) ---#
  {
    X_full <- as.matrix(po_data_long_all_complete[, pse_covariates, drop = FALSE])
    if (is.null(sieve_dim)) {
      basisN_pre <- ceiling((nrow(analysis_data))^(1/3) * ncol(X_full))
    } else {
      basisN_pre <- sieve_dim
    }
    basisN_pre <- basisN_pre + 1L
    interaction_order_pre <- if (is.null(sieve_interaction)) 2L else sieve_interaction

    sieve_basis_full <- as.data.frame(
      Sieve::sieve_preprocess(X_full, basisN = basisN_pre,
                              interaction_order = interaction_order_pre,
                              type = "cosine")$Phi
    )
    rm(X_full)

    # Column counts are constant across time cuts:
    #   po_data_long_all_complete cols + 3 S_t_pred cols added before sieve cbind
    num_covs_pre_sieve  <- ncol(po_data_long_all_complete) + 3L
    num_covs_post_sieve <- num_covs_pre_sieve + ncol(sieve_basis_full)
  }

  TMLE_final_all <- vector("list", length(clean_data$evt_times_uni))
  for (t in seq_along(clean_data$evt_times_uni)){
    ...
    #--- Free consumed list element ---#
    Surv_t_pred_long_all_list_complete[t] <- list(NULL)
    rm(po_data_t_preds_complete_all_TMLE)
    gc()
    ...
    #--- Subset pre-computed sieve basis to current time window ---#
    row_mask    <- po_data_long_all_complete$time <= clean_data$evt_times_uni[t]
    sieve_basis <- sieve_basis_full[row_mask, , drop = FALSE]
    rownames(sieve_basis) <- NULL
    ...
    while (TMLE_iter_complete == 0){
      if (t_step == 1){
        ...
        targeting_data$S_k_pred_a_star <- ifelse(targeting_data$A == 1,
                                                  targeting_data$S_k_pred_1_star,
                                                  targeting_data$S_k_pred_0_star)
        targeting_data$h_k_pred_a_star <- ifelse(targeting_data$A == 1,
                                                  targeting_data$h_k_pred_1_star,
                                                  targeting_data$h_k_pred_0_star)
      ...
      if (target_option == "Lasso - Linear - Option 1"){
        ...
        for (tp in 1:t){
          ...
          if (dim(training_data_at_risk)[1] != 0 & t_step == 1){
            ...
            if (length(unique(training_data_at_risk$Y2)) == 1){
              warning(paste0("Targeting (Option 1): Y2 is constant (all ",
                             unique(training_data_at_risk$Y2),
                             ") at time point ", clean_data$evt_times_uni[tp],
                             " (index ", tp, "). Skipping lasso - no update applied at this time point."))
              lasso_cov_list[[tp]] <- character(0)
            } else {
          ...
          if (length(linear_mod_covs) != 0){
            ...
            eps_data <- setNames(
              as.data.frame(as.list(as.numeric(eps))),
              paste0("V", seq_along(eps), "_coef_est")
            )
            ...
            #Adding in coefficient estimates (broadcast scalar values — avoids merge overhead)
            for (nm in names(eps_data)) training_data[[nm]] <- eps_data[[1L, nm]]
          ...
          result_list[[tp]] <- training_data
        }

        targeting_data <- do.call(rbind, result_list)
        ...
        if (t_step > 30){
          stop("TMLE did not converge within 30 iterations")
        }
        ...
        targeting_data <- targeting_data %>%
          ...
          group_by(ID) %>%
          mutate(
            H_k_pred_1_star = cumsum(h_k_pred_1_star),
            H_k_pred_0_star = cumsum(h_k_pred_0_star)
          )
        ...
        #--- Saving final iteration output ---#
        if (TMLE_iter_complete == 1){
          max_tps <- max(unique(targeting_data$time))
          TMLE_final_all[[t]] <- subset(targeting_data, targeting_data$time == max_tps)
        }
      }
      ...
      if (target_option == "Lasso - Linear - Option 2"){
        ...
        for (tp in 1:t){
          ...
          if (dim(training_data_at_risk)[1] != 0 & length(unique(training_data_at_risk$Y2)) > 1){
          ...
          else if (dim(training_data_at_risk)[1] != 0 & length(unique(training_data_at_risk$Y2)) == 1){
            warning(paste0("Targeting (Option 2): Y2 is constant (all ",
                           unique(training_data_at_risk$Y2),
                           ") at time point ", clean_data$evt_times_uni[tp],
                           " (index ", tp, "). Skipping lasso - no update applied at this time point."))
      ...
      if (target_option == "Lasso - Linear - Option 3"){
        ...
        for (tp in 1:t){
          ...
          if (dim(training_data_at_risk)[1] != 0 & length(unique(training_data_at_risk$Y2)) > 1){
            ...
            warning(paste0("Targeting (Option 3): Y2 is constant (all ",
                           unique(training_data_at_risk$Y2),
                           ") at time point ", clean_data$evt_times_uni[tp],
                           " (index ", tp, "). Skipping lasso - no update applied at this time point."))
          ...
          if (length(nonzero_indices) != 0){
            #Creating dataset
            eps_data <- setNames(
              as.data.frame(as.list(nonzero_covariates_val)),
              paste0(nonzero_covariates_names, "_coef")
            )
    ...
    #--- Free large objects before next time-cut ---#
    rm(po_data_long_complete_all_TMLE, sieve_basis, row_mask)
    if (exists("targeting_data")) rm(targeting_data)
    if (exists("targeting_data_prev")) rm(targeting_data_prev)
    gc()
  }
  #--- Pulling together final dataset for predictions ---#
  pred_dataset_all <- do.call(rbind, TMLE_final_all)
  ...
  #       #if an error occurs, tell me the error
  #       error=function(e) {
  #         stop(paste("An error occurred when fitting the pseudo outcome model in split ",i,sep=""))
  #         print(e)
  #       }
  #     )
  ...
  #   stop("Inappropriate pseudo-outcome regression method for obtaining CI's: pse_method must be \"Random forest\" when CI = TRUE and pse_approach = \"Pooled\"")
  # }
  ...
  if ((pse_approach == "Pooled - Factor" | pse_approach == "Pooled - Continuous") &  CI == TRUE){
    output <- list(TMLE_output=TMLE_final_all,
...
source("C:/Users/Matthew/OneDrive - University College London/Documents/Projects/surv-iTMLE/Github_repo/surv-iTMLE/src/Data_management_surv.R")
source("C:/Users/Matthew/OneDrive - University College London/Documents/Projects/surv-iTMLE/Github_repo/surv-iTMLE/src/Nuisance_models_surv.R")
